...
class _AccountPageState extends State<AccountPage> {
  ...
  Future<void> _changePhoto(ImageSource source) async {
    try {
      final file = await _picker.pickImage(source: source, imageQuality: 75, maxHeight: 1200, maxWidth: 1200);
      if (file == null) {
        return;
      }
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('No se pudo actualizar la foto: $error')));
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<AuthCubit, AuthState>(
      ...
      builder: (context, state) {
        ...
        final loadError = state.lastAction == AuthAction.loadProfile ? state.errorMessage : null;
        if (!_profileRequested && user != null && profile == null && !isLoadingProfile) {
                  ...
                  : Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(loadError ?? 'No pudimos cargar tu perfil en este momento.', textAlign: TextAlign.center),
                          const SizedBox(height: 16),
                          FilledButton(
                            onPressed: () {
                ...
                Card(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('Detalles del perfil', style: Theme.of(context).textTheme.titleMedium),
                        const SizedBox(height: 12),
                        _AccountField(label: 'Biografía', value: profile.bio),
                        const Divider(),
                        _AccountField(label: 'Ubicación', value: profile.location),
                        const Divider(),
                        _AccountField(
                          label: 'Habilidades',
                          value: profile.skills.isNotEmpty ? profile.skills.join(', ') : 'Sin habilidades registradas',
                        ),
                        const Divider(),
                        _AccountField(
                          label: 'Enlaces',
                          value: profile.links.isNotEmpty ? profile.links.entries.map((e) => '${e.key}: ${e.value}').join('\n') : 'Sin enlaces',
                        ),
                      ],
                    ),
                  ),
