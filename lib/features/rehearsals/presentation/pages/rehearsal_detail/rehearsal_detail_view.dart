      ...
      children: [
        ListView(
          ...
          children: [
            ...
            SetlistHeader(count: setlist.length),
            const SizedBox(height: 12),
            _SetlistItems(setlist: setlist, onDeleteSong: onDeleteSong),
          ],
        ),
        Positioned(
          right: 24,
...
class _SetlistItems extends StatelessWidget {
  const _SetlistItems({required this.setlist, required this.onDeleteSong});
    ...
    return Column(
      children: [
        for (final item in setlist)
          SetlistItemCard(
            item: item,
            subtitle: setlistSubtitleFor(item),
