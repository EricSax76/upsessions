...
import 'package:upsessions/features/musicians/application/musician_search_cubit.dart';
import 'package:upsessions/features/musicians/data/musicians_repository.dart';
import 'package:upsessions/features/musicians/domain/musician_entity.dart';
...
void main() {
  ...
  group('MusicianSearchCubit', () {
    blocTest<MusicianSearchCubit, MusicianSearchState>(
      ...
      build: () {
        when(() => repository.search(query: any(named: 'query'), limit: any(named: 'limit'))).thenAnswer((_) async => mockResults);
        return MusicianSearchCubit(repository: repository);
      },
      act: (cubit) => cubit.search(query: 'rock'),
      expect: () => [
        const MusicianSearchState(isLoading: true, query: 'rock', results: [], errorMessage: null),
        const MusicianSearchState(isLoading: false, query: 'rock', results: mockResults, errorMessage: null),
      ],
      verify: (cubit) {
    ...
    blocTest<MusicianSearchCubit, MusicianSearchState>(
      ...
      build: () {
        when(() => repository.search(query: any(named: 'query'), limit: any(named: 'limit'))).thenThrow(Exception('firestore-offline'));
        return MusicianSearchCubit(repository: repository);
      },
      act: (cubit) => cubit.search(query: 'jazz'),
      expect: () => const [
        MusicianSearchState(isLoading: true, query: 'jazz', results: [], errorMessage: null),
        MusicianSearchState(
          isLoading: false,
          query: 'jazz',
          results: [],
